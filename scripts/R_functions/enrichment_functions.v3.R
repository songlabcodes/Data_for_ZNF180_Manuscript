...
#### top term summary
generate_top_term.barplot <- function(data.res)
{
  data.res$corrected.FET.pvalue = p.adjust(data.res$FET_pvalue,"BH")
  require(ggplot2)
  ...
  bar.max = ggplot() + 
    geom_bar(data = plot.data,aes(x = factor(set1_Name),y = -log10(corrected.FET.pvalue)),stat = "identity") + 
    geom_label(data = subset(plot.data,corrected.FET.pvalue < 0.05),
               y = max(-log10(plot.data$corrected.FET.pvalue))/2,aes(x = factor(set1_Name),label = set2_Name)) + theme_bw() + 
    labs(x = "Cluster",y = "-log10(FET FDR)") +
